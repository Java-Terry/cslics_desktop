...
def on_pic_button_clicked():
    ...
    if pic_tab.preview_check.isChecked() and rec_button.isEnabled():
        switch_config("still")
        # print('debug pic press')
        picam2.capture_request(signal_function=qpicamera2.signal_done)
        # print('printing job get_results')
        # image = job.get_result() # this is not working
        # print('getting image?')
        # print(image)
    ...
    # TODO here, need to get image and store it into memory
    # trigger counting script/function
    # alternatively, we search for the image just written to file after .5 second delay
...
def capture_done(job):
    ...
    if not pic_tab.hdr.isChecked():
        ...
        if pic_tab.filetype.currentText() == "raw":
            ...
            # print('adding make another image')
        ...
        global image_package # HACK unsure if the only way to get the image out of here is via a global variable...
        # request = picam2.wait(job)
...
class picTab(QWidget):
    def __init__(self):
        ...
        self.count_button.clicked.connect(self.count_spawn)
        self.count_button.setEnabled(True)

        # added for cslics desktop
        # there is the global image_package from capture_done
        self.image = []
        self.metadata = []
        ...
        self.layout.addRow(self.apply_button)
        self.layout.addRow(self.count_button)
    ...
    def count_spawn(self):
        ...
        self.detector.save_image_predictions(pred, self.image, self.image_name, img_save_dir)
        self.detector.save_text_predictions(pred, self.image_name, txt_save_dir)
